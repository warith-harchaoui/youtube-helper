...
def test_mcp_module_imports_and_exposes_app():
    ...
    from youtube_helper import mcp as mcp_module

    assert hasattr(mcp_module, "app"), "youtube_helper.mcp must re-expose `app`."
    assert hasattr(mcp_module, "mcp"), "youtube_helper.mcp must expose the `mcp` handler."


def test_main_entrypoint_is_callable():
    """The ``youtube-helper-mcp`` console entry point should be a callable."""
    from youtube_helper.mcp import main
