...
« télécharger et inspecter de la vidéo ou de l'audio en ligne », comparés
à `youtube-helper`. Les notes vont de ⭐ (1) à ⭐⭐⭐⭐⭐ (5), évaluées sur la
tâche visée par `youtube-helper` : le téléchargement quotidien YouTube,
Vimeo, DailyMotion et Twitch, plus les métadonnées d'engagement sans API,
pour le personal branding et les pipelines d'IA. Un outil optimisé pour
un tout autre usage, l'archivage miroir ou la rediffusion par torrent par
exemple, n'est pas pénalisé : la note reflète seulement l'adéquation à
*ce* créneau.
...
La carte est un résumé en 2D des 8 critères : à lire comme une forme, pas comme un classement. « youtube-helper » se situe dans le coin en haut à droite. Les axes se lisent **Horizontal — Passion ↔ Efficacité** et **Vertical — Accessibilité ↔ Innovation**.
<!-- FIGURE:END -->

## Positionnement

`youtube-helper` se place volontairement à l'intersection de la
**couverture brute de yt-dlp** et de l'**ergonomie des pipelines d'IA et
de personal branding**. Il ne cherche pas à concurrencer yt-dlp sur
...
- **Formes de retour normalisées** : les métriques d'engagement suivent
  le même schéma sur YouTube, Vimeo, DailyMotion et Twitch, si bien que
  le code en aval n'a besoin d'aucune branche par site. C'est l'écart
  derrière la note « info dict brut » de yt-dlp : la donnée est là, mais
  non normalisée.
- **Résolveur d'URL directe, catalogue de flux et sélecteur** : transforme
  la liste `formats` brute de yt-dlp en un objet de première classe avec
  des filtres de codec, de fps et de langue, prêt à passer à
  `ffmpeg` ou à `video-helper.extract_frames`.
- **Métadonnées de branding sans API** : instantanés au niveau de la
  ...
  média et coûte l'OAuth et le quota.
...
- **Prêt pour Docker** : un seul `docker run` suffit pour un serveur
  HTTP local sur n'importe quelle URL prise en charge par yt-dlp.

## Quand choisir quoi

- **`youtube-helper`** : téléchargement et métadonnées d'engagement pour
  les pipelines d'IA et de personal branding, surtout quand on veut le
  même outillage en bibliothèque, CLI et HTTP.
- **`yt-dlp`** (direct) : vous possédez déjà le pipeline et il vous faut
  seulement l'info dict brut ou les fichiers, sans couche de normalisation.
...
- **`google-api-python-client`** : vous avez besoin des compteurs
  canoniques de vues, de likes ou d'abonnés, de données de rétention ou
  d'analytique ; ou bien vous payez déjà le coût de l'OAuth et du quota.
- **`streamlink`** : flux de travail orienté direct ; vous voulez passer
  une URL live directement à un lecteur sans enregistrer.
- **`gallery-dl`** : téléchargements en masse de galeries d'images ou de
...
- **`scrapetube`** : vous n'avez besoin que d'énumérer les identifiants
  de vidéos d'une chaîne, d'une playlist ou d'une recherche sans l'API
  Data, sans téléchargement.
