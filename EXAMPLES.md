...
and that `yt-dlp` and `ffmpeg` are installed (`brew install yt-dlp ffmpeg`
on macOS, install `brew` from [brew.sh](https://brew.sh/) first).
...
Where `download_*` writes a file, the catalog and picker functions hand
you a **direct media URL** plus headers, so a downstream decoder (PyAV,
ffmpeg, `video_helper.extract_frames`) can read it without going back
through yt-dlp.
...
# Pick the one matching your constraints: the best yt-dlp candidate among matches.
...
**Note on audio**: a parallel audio catalog and picker intentionally lives
in **podcast-helper**, the suite's single owner for audio PCM streaming, not here.
...
Coarser than `pick_video_stream` (no codec, format, or fps filtering),
but a single call.

## Composing with video-helper

The whole point of the catalog and picker: feed the direct URL and headers to
...
call: it runs `pick_video_stream` under the hood, splices in the
...
pipeline, just by changing `destination`, `device`, or `batch_size`.
The picker is invoked once and the resolved stream is reused for the
entire iteration.

> **Heads-up**: `extract_frames_stream` needs a `video-helper` release with
> URL-aware `is_valid_video_file` and `video_dimensions(http_headers=)`;
> youtube-helper's `pyproject.toml` pins `video-helper>=2.0.0,<3` for this.
...
# Channel video list with per-video engagement (paginated, slow: one fetch per video)
...
# Batched engagement across a list of URLs: tolerant, bad URLs get an _error stub
...
```python
# Auto-subtitles, returns {lang: path_to_vtt}
...
comments = yth.video_comments(
    "https://www.youtube.com/watch?v=YE7VzlLtp-4",
    max_count=50,
    cookies_from_browser="firefox",   # or "chrome" / "safari", reads your active session
)
for c in comments[:3]:
...
# Heuristic: is this a Short, a vertical clip?
...
- **URL expiration**: direct media URLs from yt-dlp expire, typically
  within one to six hours for YouTube. For long-running jobs, re-resolve
  periodically, or use `download_video` to materialize a local file.
- **Live streams**: `pick["is_live"] == True` means `pick["url"]` is
  typically an HLS `.m3u8` manifest. ffmpeg and PyAV handle these
  natively, but with no seek backwards, no `frame_indices` or `frame_times`
  semantics, and hwaccel can be flaky with chunked HLS. A future
  `extract_frames_stream` will refuse `speed != 1.0` for live streams,
  since you cannot fast-forward past the live edge.
...
- **Authentication**: members-only, age-gated, or private videos need
  cookies. Pass `cookies_from_browser="firefox"` (or `"chrome"` or
  `"safari"`) to the catalog, picker, or branding functions; yt-dlp
  reads the active browser session.
- **Comments scraping**: YouTube comment threads almost always need
  `cookies_from_browser` now. Without it you will usually get an empty
  list or a 403.
