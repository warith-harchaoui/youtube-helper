...
8. **`settings.yaml.example`** for any helper that loads credentials via
   `os_helper.get_config`. Real `settings.yaml` files are `.gitignore`d;
...
Cross-helper dependency pins are version ranges against the PyPI
releases (e.g. `video-helper>=2.0.0,<3`), the same as any other
dependency.

## CI

GitHub Actions runs `pytest` on Python 3.12 (Linux), a deliberately
light single-version gate; the full multi-version sweep runs locally
before push. A blocking `ruff check` and `ruff format --check` job
runs alongside it. Integration tests are marked `integration` and
...
[BSD-3-Clause](LICENSE), the same license as scikit-learn, numpy, and scipy.
